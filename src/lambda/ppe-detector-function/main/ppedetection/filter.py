...
def filter_result(ppe_res: dict, min_confidence: int) -> dict:
    """
    ...
    incompliant_list = ppe_res["Summary"]["PersonsWithoutRequiredEquipment"] # List of Id of people violating PPE guidelines
    ...
    resp["Summary"] = {}
    resp["Summary"]["SumPeopleWithRequiredEquipment"] = len(ppe_res["Persons"]) - len(incompliant_list)
    resp["Summary"]["SumPeopleWithoutRequiredEquipment"] = len(incompliant_list)

    logger.info(f'{len(incompliant_list)} people detected without protective equipment')
    logger.info(
